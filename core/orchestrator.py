...
from core.actionable_generator import generate_actionable
...
# Skills runnable in parallel after B1 has completed.  B5 (seller-side
# failure) intentionally removed — the available seller data is for
# astbuy chase-routing, not for the buyer-recommendation pool that B5
...
def run_rca(ctx: dict) -> dict:
    ...
    writer.save("05_skill_spec_quality", results["spec_quality"])
    writer.save("06_skill_quantity",     results["quantity"])
    writer.save("07_skill_retail",       results["retail"])
    ...
    # ── Final synthesis: one LLM pass over everything to produce actionable ──
    actionable = {}
    try:
        actionable = generate_actionable(ctx, {
        ...
        logger.exception("actionable generation failed")
        actionable = {"error": str(e)}
    ...
    writer.save("08_actionable", actionable)
